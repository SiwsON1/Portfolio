...
const Skills = () => (
  <div className="p-8">
    ...
    {/* SEO Skills Section */}
    <div className="relative flex flex-col p-4 mt-10 w-full h-auto flex-shrink-0 rounded-lg bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-50 border border-gray-100">
      <h2 className="text-2xl text-center font-bold mb-5">SEO Skills</h2>
      <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-y-4 justify-items-center">
        {skills.seo.map((skill, index) => (
          <li key={index} className="text-center text-lg font-semibold">
            {skill}
          </li>
        ))}
      </ul>
